...
export const SeasonalVegetableShowcase: React.FC<SeasonalVegetableShowcaseProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="bg-gradient-to-br from-emerald-50 via-amber-50/50 to-orange-50 border border-emerald-200 rounded-2xl p-6 mb-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-stone-600 mt-1 max-w-3xl font-medium">
          ...
          {/* Quick Stat Pill */}
          <div className="flex items-center gap-3 bg-white p-3.5 rounded-2xl border border-emerald-200 shadow-xs shrink-0">
            ...
            <div>
              ...
              <div className="text-xs text-stone-600 font-semibold">Peak In-Season Vegetables</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full bg-white border border-stone-300 rounded-xl pl-9 pr-4 py-2 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-xs"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {filteredVegetables.map((veg) => {
          ...
          return (
            <div
              key={veg.id}
              className={`bg-white border rounded-2xl p-5 flex flex-col justify-between transition-all hover:shadow-md ${
                isPeak
            ...
            >
              <div>
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isPeak && (
                        <span className="px-2 py-0.5 text-[10px] font-bold uppercase rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300">
                          Peak Season
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-orange-700 font-bold">{veg.hindiName}</p>
                    <p className="text-[11px] text-stone-400 italic font-serif">{veg.botanicalName}</p>
                  </div>
                  ...
                  <div className={`p-2 rounded-xl text-xs flex items-center gap-1 shrink-0 ${
                    ...
                    {isCooling ? <Droplet className="w-3.5 h-3.5 text-sky-600" /> : <Flame className="w-3.5 h-3.5 text-amber-600" />}
                    <span className="text-[10px] font-bold">{veg.ayurvedicProperties.virya.split('(')[0]}</span>
                  </div>
                </div>

                {/* Multilingual Regional Names Pill Drawer */}
                <div className="bg-stone-50/80 border border-stone-200 rounded-xl p-3 mb-3 text-xs">
                  <div className="flex items-center gap-1 text-[11px] text-stone-600 font-bold mb-1.5">
                    <Languages className="w-3.5 h-3.5 text-orange-600" />
                    <span>Regional Names in India:</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5 text-[11px]">
                    {veg.regionalNames.tamil && (
                      <span className="px-2 py-0.5 rounded-lg bg-white border border-stone-200 text-stone-700">
                        <strong className="text-stone-500 font-semibold">Tamil:</strong> {veg.regionalNames.tamil}
                      </span>
                    )}
                    {veg.regionalNames.telugu && (
                      <span className="px-2 py-0.5 rounded-lg bg-white border border-stone-200 text-stone-700">
                        <strong className="text-stone-500 font-semibold">Telugu:</strong> {veg.regionalNames.telugu}
                      </span>
                    )}
                    {veg.regionalNames.bengali && (
                      <span className="px-2 py-0.5 rounded-lg bg-white border border-stone-200 text-stone-700">
                        <strong className="text-stone-500 font-semibold">Bengali:</strong> {veg.regionalNames.bengali}
                      </span>
                    )}
                    {veg.regionalNames.marathi && (
                      <span className="px-2 py-0.5 rounded-lg bg-white border border-stone-200 text-stone-700">
                        <strong className="text-stone-500 font-semibold">Marathi:</strong> {veg.regionalNames.marathi}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs py-2 border-y border-stone-100 mb-3 text-stone-600">
                  <span>
                    Digestion: <strong className="text-emerald-700 font-bold">{veg.ayurvedicProperties.digestionEase}</strong>
                  </span>
                  <span className="text-orange-800 font-semibold text-right">
                ...
                <div className="text-xs text-stone-600 mb-4 bg-orange-50/50 p-2.5 rounded-xl border border-orange-100">
                  <span className="font-bold text-orange-950 block mb-1">Everyday Pantry Pairings:</span>
                  <div className="flex flex-wrap gap-1">
                    {veg.pantryPairings.map((pairing, i) => (
                      <span key={i} className="px-2 py-0.5 rounded-md bg-white border border-orange-200 text-orange-900 text-[11px] font-medium">
                        {pairing}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-stone-100">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white shadow-xs transition-all cursor-pointer"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl text-xs font-bold bg-orange-50 hover:bg-orange-100 text-orange-800 border border-orange-200 transition-all cursor-pointer flex items-center gap-1"
